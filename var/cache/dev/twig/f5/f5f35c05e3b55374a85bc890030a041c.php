...
class __TwigTemplate_5afe1dd6844ba525ad7d6f2ad3e4a034 extends Template
{
    ...
    <div class=\"modal-content\">
      ...
      <div class=\"modal-body\">
        <div id=\"modalNewProductBody\">
          <!-- il form verrà caricato qui via AJAX -->
      ...
      <div class=\"modal-footer\">
        <!-- lasciare vuoto: il form includerà il proprio submit -->
...
/* ---------- helper: resolveProductIdForExtra + attachBubbleClickHandler ---------- */
function resolveProductIdForExtra(p) {
  if (!p) return null;

  // 1) preferisci prodottoId esplicito
  ...
  // helper: estrae prima sequenza di cifre trovata
  ...
  try {
    if (window.__fullcalendar_instance && typeof window.__fullcalendar_instance.getEvents === 'function') {
      ...
      // cerca evento con extendedProps.prodottoId uguale a candFields espliciti
      for (const ev of evs) {
      ...
      // cerca evento con extendedProps.stockEntryId uguale
      for (const ev of evs) {
        const ep = ev.extendedProps || {};
...
async function initProductFormWidgets(container) {
  ...
  // utility: assicurati che l'input hidden esista (usato per colore/emoji)
...
// --- 1) CATEGORIA \"ALTRO\" (versione robusta: rimuove style inline sull'input) ---
try {
  // trova la select della categoria con varie euristiche
  ...
  // trova l'input per categoria custom (diverse forme possibili)
  ...
  // wrapper: cerca un container logico; se non c'è useremo il parent
\tconst altroWrap = document.getElementById('categoriaCustomWrap') || (altroInput ? altroInput : null);

function showAltro() {
  // preferisci mostrare il wrapper separato, altrimenti mostra solo l'input
  ...
  if (altroInput) {
    ...
    // rimuovi style inline che potevano nasconderlo
...
function hideAltro() {
  ...
  if (cat) {
    // evita doppie attach riconoscendo una flag sul select
  ...
  try {
    ...
    // inizializza colore da valore server (se presente)
    try {
      const serverVal = (function(){ try { return ";
        // line 1052
        yield (((array_key_exists("form", $context) &&  !(null === CoreExtension::getAttribute($this->env, $this->source, CoreExtension::getAttribute($this->env, $this->source, CoreExtension::getAttribute($this->env, $this->source, (isset($context["form"]) || array_key_exists("form", $context) ? $context["form"] : (function () { throw new RuntimeError('Variable "form" does not exist.', 1052, $this->source); })()), "colore", [], "any", false, false, false, 1052), "vars", [], "any", false, false, false, 1052), "value", [], "any", false, false, false, 1052)))) ? (json_encode(CoreExtension::getAttribute($this->env, $this->source, CoreExtension::getAttribute($this->env, $this->source, CoreExtension::getAttribute($this->env, $this->source, (isset($context["form"]) || array_key_exists("form", $context) ? $context["form"] : (function () { throw new RuntimeError('Variable "form" does not exist.', 1052, $this->source); })()), "colore", [], "any", false, false, false, 1052), "vars", [], "any", false, false, false, 1052), "value", [], "any", false, false, false, 1052))) : ("null"));
...
/* sostituisci la tua funzione selectColor con questa (o sovrascrivila) */
let current = null;
function selectColor(hex, btnEl){
  try {
    ...
    // aggiorna sempre l'etichetta: usa nameFromHex se disponibile, altrimenti mostra l'hex
    if (hexLabel) hexLabel.textContent = (typeof nameFromHex === 'function' ? nameFromHex(hex) : hex);
...
// se esiste un hidden che può essere cambiato da plugin, osserviamolo e aggiorniamo UI
try {
  const observedHidden = container.querySelector('input[name=\"prodotto[colore]\"], #colorHidden');
  if (observedHidden) {
    // se il valore viene modificato direttamente, aggiorniamo preview e label
    ...
    // MutationObserver per catturare assegnazioni dirette che non triggerano 'input'
...
// assicurati che ogni swatch/grigla chiami selectColor (se non l'hanno già)
try {
  if (colorGrid) {
  ...
  // --- 4) EMOJI PICKER (usa emoji-picker-element dinamico come nel tuo form) ---
  try {
    ...
    async function openEmojiPickerLocal() {
      ...
      try {
        // import dinamico del webcomponent emoji-picker (funziona qui poiché chiamato dopo inject)
        if (!window.customElements.get('emoji-picker')) {
          await import('https://cdn.jsdelivr.net/npm/emoji-picker-element@^1/index.js');
        }
        // rimuovi eventuali picker esistenti
  ...
  // --- 5) ensure hidden inputs are present on submit (colore & emoji) ---
  try {
...
async function openNewProductModal() {
  ...
  try {
    ...
    // reinizializza widget e ottieni il form
    let newForm;
    try {
      newForm = initProductFormWidgets(modalBody);
      // se initProductFormWidgets restituisce una Promise, attendila
    ...
    // fallback: cerca il primo form dentro il modalBody
    if (!(newForm instanceof HTMLFormElement)) {
      newForm = modalBody.querySelector('form');
    }

    // se ancora non trovato -> messaggio d'errore user-friendly
    if (!newForm) {
    ...
    // assicurati che abbia action valido
    ...
    // evita di bindare più volte lo stesso listener
    ...
    // (opzionale) controlla messaggi di errore server-side e segnala UI
    try {
...
async function submitProductFormAJAX(form, bsModal) {
  ...
  // opzione: mostra loader nel pulsante submit
  ...
  try {
    ...
    if (resp.ok && resp.headers.get('content-type') && resp.headers.get('content-type').includes('application/json')) {
      ...
      if (data.success) {
        ...
        // caso particolare: server ritorna success=false e html? gestisci come vuoi
        console.warn('Risposta JSON senza success=true', data);
      }
    }

\t// Se validazione fallita: backend ritorna HTML del form (status 422 o 400)
\tif (resp.status === 422 || resp.status === 400 || (resp.headers.get('content-type') && resp.headers.get('content-type').includes('text/html'))) {
\t  const html = await resp.text();
\t  // re-inserisci l'html del form (contenente errori)
\t  const modalBody = document.getElementById('modalNewProductBody');
\t  modalBody.innerHTML = html;

\t  // reinizializza widget e ottieni nuovo form
\t  const newForm = initProductFormWidgets(modalBody);

\t  // ri-attach submit listener
\t  if (newForm) {
\t\tif (!newForm.action || newForm.action.trim() === '') {
\t\t  newForm.action = window.__newProductUrl || '";
        // line 1463
...
async function openProductShowModalById(prodId) {
  ...
  // costruisci lista candidate URL (in ordine di preferenza)
  const candidates = [];

  // 1) se Twig ha esposto template con id=0, sostituisci SOLO quella occorrenza di 0 che rappresenta l'id
  try {
    if (window.__prodFragmentUrlTemplate) {
      // sostituiamo '0' alla fine o '/0/' prima di 'fragment' - più robusto: replace '/0' seguito da '/' o end
  ...
  // 2) fallback su percorsi relativi (in caso tu non abbia inserito i template Twig)
  candidates.push('/prodotto/' + encodeURIComponent(prodId) + '/fragment');
  candidates.push('/prodotto/' + encodeURIComponent(prodId));

  // 3) fallback con basePath se fornito da Twig
  if (window.__appBasePath) {
    // assicurati di non duplicare slash
  ...
  // de-dup e rimozione di eventuali vuoti
  const triedSet = new Set();
  const urls = [];
  for (const u of candidates) {
    if (!u) continue;
    const clean = u.replace(/\\/+/g, '/'); // semplifica doppie barre
    if (!triedSet.has(clean)) { triedSet.add(clean); urls.push(clean); }
  }

  // helper fetch HTML con header AJAX
  ...
  // prova ogni URL fino a successo
  let lastErr = null;
  for (const url of urls) {
    ...
    try {
      ...
      // esegui script inline se presenti (semplice reinserimento)
      ...
      // inizializza widget interni al fragment (se hai la funzione)
      ...
      return; // trovato e caricato con successo
    } catch (err) {
  ...
  // se siamo qui, nessun URL ha funzionato
...
";
    ...
    {
        return array (  1628 => 1463,  1540 => 1378,  1495 => 1336,  1208 => 1052,  1110 => 957,  348 => 198,  338 => 192,  325 => 191,  309 => 185,  283 => 162,  248 => 129,  235 => 128,  102 => 6,  89 => 5,  66 => 3,  43 => 1,);
    }
...
{% block title %}Benvenuto — Dispensa{% endblock %}
...
<script>
  window.__newProductUrl = '{{ path(\"app_prodotto_new\") }}';
</script>
...
<script>
\twindow.__showProductUrlTemplate = '{{ path(\"app_prodotto_fragment\", {\"id\": 0})|escape(\"js\") }}';
</script>
...
{% block javascripts %}
    ...
    (function(){
        const PRIMARY_JS = 'https://cdn.jsdelivr.net/npm/fullcalendar@6.1.19/index.global.min.js';
        const FALLBACK_JS = 'https://unpkg.com/fullcalendar@6.1.19/index.global.min.js';
        const EVENTS_URL = '{{ path('api_calendar_events') }}';
...
async function initProductFormWidgets(container) {
  ...
  // fallback action
  if (!form.action || form.action.trim() === '') {
    form.action = window.__newProductUrl || '{{ path(\"app_prodotto_new\") }}';
  }
...
function hideAltro() {
  ...
  try {
    ...
    // inizializza colore da valore server (se presente)
    try {
      const serverVal = (function(){ try { return {{ form is defined and form.colore.vars.value is not null ? form.colore.vars.value|json_encode|raw : 'null' }}; } catch(e){ return null; } })();
      let hex = null;
      if (serverVal) hex = (typeof serverVal === 'string') ? serverVal : null;
      if (!hex) hex = '#ffffff';
      const colorHiddenEl = container.querySelector('#colorHidden');
...
async function openNewProductModal() {
  const modalEl = document.getElementById('modalNewProduct');
  const modalBody = document.getElementById('modalNewProductBody');
  const url = window.__newProductUrl || '{{ path(\"app_prodotto_new\") }}';
  ...
  try {
    ...
    // assicurati che abbia action valido
    if (!newForm.action || newForm.action.trim() === '') {
      newForm.action = window.__newProductUrl || '{{ path(\"app_prodotto_new\") }}';
    }
...
{% endblock %}
", "home/index.html.twig", "C:\\xampp\\htdocs\\pantry\\templates\\home\\index.html.twig");
    }
}
